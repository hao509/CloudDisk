...
import com.example.clouddisk.xss.SQLFilter;
...
public class Query<T> {
    ...
    public IPage<T> getPage(Map<String, Object> params, String defaultOrderField, boolean isAsc) {
        ...
        if(StringUtils.isNotEmpty(orderField) && StringUtils.isNotEmpty(order)){
            String[] orderFieldArr = orderField.split(",");
            ...
            for (int i = 0; i < orderFieldArr.length; i++) {
                ...
                if (Constant.ASC.equalsIgnoreCase(asc)){
                    page.addOrder(OrderItem.asc(orderFieldArr[i]));
                }else{
                    page.addOrder(OrderItem.desc(orderFieldArr[i]));
                }
            }
        }
        //前端字段排序
